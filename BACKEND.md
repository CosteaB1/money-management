...
- **Reconciliation tripwire** (`PoolReconciliation`) - replays the ledger against the account and **reports; never corrects, never throws.** Each finding has more than one possible cause and only the user knows which record is wrong; a handler that "repaired" the cheaper side would destroy the evidence that a third party's stake had been mispriced. **Five checks: unaccounted money** (cash the guards should have blocked); **`sum(participantUnits) == totalUnits`**; **recorded vs. re-derived pre-money** (marks are folded *forward*, grouped per day and ordered by UUIDv7 write order, so a snapshot taken later on the same day as a priced event is not mistaken for drift); **unbacked cash claims** (a unit event claiming money the account never saw); and **the balance identity** (added 2026-09-07 - see below). The tautology `sum(units) x nav == poolValue` is **not** checked: `nav` is *defined* as `poolValue / units`, so it holds by construction and proves nothing.
- **The balance identity, and why it had to exist** (`CheckBalanceIdentity`). A real pool reached **3,000 units against a 2,000 balance, NAV 0.667** - one back-dated arrival never got its transaction written - and the tripwire reported **clean**. Two defects: the unbacked-claim matcher paired a claim to *any* account row with the same `(Date, Direction, Amount)`, so the friend's phantom claim was consumed by the **owner's own funding transfer**; and nothing compared the ledger's implied balance to the real one. Fixed by (a) reserving the seed's backing as a *budget* before the walk - `surplus = balanceAsOf(inception) - linked rows <= inception - seedValue`, which is exactly zero on a healthy pool because `CreatePool`'s catch-up mark makes it so - and (b) predicting the balance from the anchor plus post-inception marks plus settled event cash, and reporting `PredictedBalance` / `DerivedBalance` / `BalanceDrift` / `BalanceReconciles`. **The anchor is derived from the account, never read off the seed event**: anchoring on the seed looks equivalent but false-positives on an ordinary same-day create-then-subscribe, where `PoolMark` writes a mark dated *on* the inception date after the seed was struck. Unpaid distributions are excluded via `SettledOn is null` (that cash has not left), and pre-inception history sits inside the anchor so it is never double-counted. One redundancy accepted deliberately: a stray manual row now trips both this check and `UnmatchedTransactions` - two findings, one cause.
...
- **Three exits, added 2026-09-07 after a real dead end.** Archiving was originally one-way on the reasoning that an archived pool's account is wholly the owner's again. That holds for winding down a real pool and fails for one created **by mistake**: it could not be deleted, its seed could not be deleted (`pools.seed_cannot_be_deleted`), and because `pools.account_id` is `RESTRICT` **and** `ix_pools_account_id` is unique and unfiltered on `is_archived`, the dead row went on holding its account forever *and* stopped that account from ever carrying another pool. So: `DELETE /pools/{id}` removes a pool with no non-owner units and no unit event carrying cash or a movement transaction — the created-by-mistake shape, which provably cannot destroy a third party's stake — else **409 `pools.delete_has_movements`**; and `POST /pools/{id}/unarchive` is an idempotent flip. Both read with `IgnoreQueryFilters` because the stuck rows are precisely the archived ones. **Neither touches the account's transactions**: a pool's catch-up mark genuinely moved the balance and may already have been reconciled against, so reversing it silently would rewrite balance history. `DeleteAccount` still counts archived pools — skipping them would not make the account deletable, it would just turn a clean 409 back into the FK 500 the pre-check exists to prevent.
